...
public abstract class EventListenerService<TService, TTelemetryConsumer, TMetrics> : EventListener, IHostedService
    where TMetrics : class, new()
{
    ...
    private void EnableEventSource(EventSource eventSource)
    {
        ...
        var arguments = enableMetrics ? new Dictionary<string, string?> { { "EventCounterIntervalSec", "1" } } : null;
