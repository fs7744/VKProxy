...
public class ACMECommandOptions
{
    ...
    {
        command.AddArg(new CommandArg("server", "s", null, $"The dictionary URI to an ACME server. (default is test server: {WellKnownServers.LetsEncryptStagingV2})", s => command.Args.Server = new Uri(s), check: () => command.Args.Server != null));
